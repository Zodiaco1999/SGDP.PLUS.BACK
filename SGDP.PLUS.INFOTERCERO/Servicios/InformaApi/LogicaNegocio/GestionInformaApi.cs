...
public class GestionInformaApi : BaseAppService, IGestionInformaApi
{
    ...
    public async Task<ConsultaLaftTerceroResponse> ConsultaLaftTercero(ConsultaLaftTerceroCommand command)
    {
        ...
        var consultasInforma = identificaciones.Select(async identificacion => await ConsultaLaft(new ConsultaLaftCommand(identificacion)));
        var consultasLaft = await Task.WhenAll(consultasInforma);

        listaConsultasLaft.AddRange(consultasLaft);
        consultasLaft.ForEach(c => ilicitos.AddRange(c.ListaIlicitos));

        await SaveRespuestaLaft(listaConsultasLaft, fechaSolicitud, command.Nit);
        int numeroOcurrencias = ilicitos.Count;
        resumenRespuesta.NumeroOcurrencias = ilicitos.Count.ToString();
